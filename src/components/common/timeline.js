...
function monthDiff(startDate, endDate) {
  ...
  let monthDuration = Math.ceil(
    (endDate_.setDate(1) - startDate.setDate(1)) / secondsInMonth
  );
  ...
  const TimeLabel = (
    <div className="hide-when-small">
      <FadeInDiv direction="right">
        <Text strong={true} style={textStyle}>
          {formatString(startDate)} -
        </Text>
      </FadeInDiv>
      <FadeInDiv direction="right">
        <Text strong={true} style={textStyle}>
          {formatString(endDate)}
        </Text>
      </FadeInDiv>
      <FadeInDiv direction="right">
        <Text style={textStyle}>({monthDiff(startDate, endDate)})</Text>
      </FadeInDiv>
    </div>
  );
